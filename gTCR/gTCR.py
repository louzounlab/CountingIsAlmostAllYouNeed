...
from gTCR.train_test_val_ktimes import TrainTestValKTimes
import json
import csv
import torch
from gTCR.TcrDataset import *
...
def results_dealing(return_lists, RECEIVED_PARAMS, result_file_name):
    ...
    print("\n \nMean minimum Validation and Train Sets AUC: ", mean_min_train_val_metric, " +- ",
          std_min_train_val_metric)
...
def run(config, params):
    ...
    config_dict = config
    params_dict = params
    config_dict["params_file_path"] = params_dict
    kwargs = config_dict
...
def set_arguments():
    ...
    parser.add_argument("--params", help="Hyper parameters file path", default="tcr_concat_graph_and_values.json",
                        type=str)
    return parser
