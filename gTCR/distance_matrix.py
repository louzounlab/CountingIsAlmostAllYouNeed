...
import gTCR.cyclic_vae_best_params_weight_cdr3 as ae
import numpy as np
import torch
...
def create_distance_matrix(device, outliers_file="outliers", autoencoder_path='vae_vgene_ae.pt', adj_mat="dist_mat"):
    ...
    ae_dict = torch.load(autoencoder_path, map_location=device)
    ...
    for tcr in outliers.keys():
        ...
        if vgene is "unknown" or vgene not in ae_dict['v_dict']:
            ...
            embeddings.append(
                embed(cdr3, vgene, ae_dict['amino_pos_to_num'], ae_dict['max_len'], ae_dict['v_dict'], device))
    encodings = []
    ...
    for emb in embeddings:
        with torch.no_grad():
            ...
            _, mu, _ = model(emb)
            encodings.append(mu)
    tcrs = [tcr for tcr in outliers.keys() if tcr.split('_')[1] != "unknown" and tcr.split('_')[1] in ae_dict['v_dict']]
    # print(tcrs)
    header = [''] + tcrs
    matrix = [header]
    for i, enc1 in tqdm(enumerate(encodings), total=len(encodings), desc="Calculate pairwise distance"):
        line = []
