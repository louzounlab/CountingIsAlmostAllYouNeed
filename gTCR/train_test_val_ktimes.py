import torch
from gTCR.concat_graph_and_values import *
from gTCR.distance_matrix import *
from gTCR.ofek_files_utils_functions import *
from gTCR.train_test_val_ktimes_utils import *
from gTCR.train_test_val_one_time import TrainTestValOneTime
...
class TrainTestValKTimes:
    ...
    def train_group_k_cross_validation(self, k=5):
        ...
        for i in range(k):
            ...
            train_idx, val_idx = train_test_split(indexes_array, test_size=0.1111, shuffle=True)
            print(f"Run {run}")
            ...
            min_train_val_metric.append(
                min_val_train_auc)  # the minimum between the aucs between train set and validation set
    ...
    def create_corr_tcr_network(self, train_idx, file_directory_path, outlier, corr_file_name, Threshold=0.2):
        ...
        for i, subject in tqdm(enumerate(train_subject_list), desc='Create corr matrix tcrs',
                               total=len(train_subject_list)):
        ...
        corr_df_between_golden_tcrs = arrange_corr_between_golden_tcr_mat(corr_df_between_golden_tcrs,
                                                                          Threshold=threshold)
        corr_df_between_golden_tcrs.to_csv(f"{corr_file_name}.csv")
        return corr_df_between_golden_tcrs

    def create_projection_tcr_network(self, outliers_pickle_name, adj_mat_path=0.2):
        proj_matrix = create_distance_matrix(self.device, outliers_file=outliers_pickle_name,
                                             autoencoder_path=self.kwargs['ae'], adj_mat=adj_mat_path)
        threshold = 0.62
